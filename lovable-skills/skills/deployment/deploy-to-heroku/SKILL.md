...
description: Use when deploying an app to Heroku. Best for: Traditional PaaS. Always configures Dynos, add-ons, buildpacks, pipelines properly.
---

# Deploy to Heroku

Production deployment guide for Heroku.

## Why Heroku
- Traditional PaaS
- Key features: Dynos, add-ons, buildpacks, pipelines

## Prerequisites
- App is containerized (or compatible with Heroku's build system)
- Environment variables defined
- Health check endpoint implemented
- Database configured (managed or external)

## Deployment Configuration
Provide the configuration files specific to Heroku:
...
## Scaling
- Heroku scaling options (horizontal, vertical, auto)
- Cost implications
- Cold start mitigation (if serverless)

## Database Strategy
- Managed DB on Heroku (if offered)
...
## Security
- HTTPS only
- Environment secrets in Heroku's secret manager
- Rate limiting
- WAF (if available)
