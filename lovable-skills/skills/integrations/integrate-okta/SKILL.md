---
name: integrate-okta
description: Use when integrating Okta authentication into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: enterprise SSO, SAML, SCIM.
---

# Integrate Okta authentication

Production integration guide for Okta authentication.

## What Okta authentication Provides
enterprise SSO, SAML, SCIM

## Setup
1. Create account / project on Okta authentication
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install okta` (or equivalent)

## SDK Setup
```ts
// lib/okta.ts
import { Client } from 'okta'
