---
name: integrate-zendesk-api
description: Use when integrating Zendesk API into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: tickets, help center, chat.
---

# Integrate Zendesk API

Production integration guide for Zendesk API.

## What Zendesk API Provides
tickets, help center, chat

## Setup
1. Create account / project on Zendesk API
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install zendesk_api` (or equivalent)

## SDK Setup
```ts
// lib/zendesk-api.ts
import { Client } from 'zendesk-api'

export const client = new Client({
  apiKey: process.env.ZENDESK_API_API_KEY!,
})
```
