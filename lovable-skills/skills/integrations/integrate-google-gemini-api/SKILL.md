---
name: integrate-google-gemini-api
description: Use when integrating Google Gemini API into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: Gemini, vision, code, multimodal.
---

# Integrate Google Gemini API

Production integration guide for Google Gemini API.

## What Google Gemini API Provides
Gemini, vision, code, multimodal

## Setup
1. Create account / project on Google Gemini API
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install google_gemini_api` (or equivalent)

## SDK Setup
```ts
// lib/google-gemini-api.ts
import { Client } from 'google-gemini-api'

export const client = new Client({
  apiKey: process.env.GOOGLE_GEMINI_API_API_KEY!,
})
```
