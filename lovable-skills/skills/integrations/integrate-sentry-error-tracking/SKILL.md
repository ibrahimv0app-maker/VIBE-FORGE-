---
name: integrate-sentry-error-tracking
description: Use when integrating Sentry error tracking into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: errors, performance, replays.
---

# Integrate Sentry error tracking

Production integration guide for Sentry error tracking.

## What Sentry error tracking Provides
errors, performance, replays

## Setup
1. Create account / project on Sentry error tracking
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install sentry_error_tracking` (or equivalent)

## SDK Setup
```ts
// lib/sentry-error-tracking.ts
import { Client } from 'sentry-error-tracking'

export const client = new Client({
  apiKey: process.env.SENTRY_ERROR_TRACKING_API_KEY!,
})
```
