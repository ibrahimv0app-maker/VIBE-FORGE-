---
name: integrate-statsig-flags
description: Use when integrating Statsig feature flags + experiments into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: flags, A/B, product analytics.
---

# Integrate Statsig feature flags + experiments

Production integration guide for Statsig feature flags + experiments.

## What Statsig feature flags + experiments Provides
flags, A/B, product analytics

## Setup
1. Create account / project on Statsig feature flags + experiments
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install statsig_flags` (or equivalent)

## SDK Setup
```ts
// lib/statsig-flags.ts
import { Client } from 'statsig-flags'

export const client = new Client({
  apiKey: process.env.STATSIG_FLAGS_API_KEY!,
})
```
