---
name: integrate-amplitude-analytics
description: Use when integrating Amplitude analytics into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: product analytics, cohorts, A/B.
---

# Integrate Amplitude analytics

Production integration guide for Amplitude analytics.

## What Amplitude analytics Provides
product analytics, cohorts, A/B

## Setup
1. Create account / project on Amplitude analytics
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install amplitude_analytics` (or equivalent)

## SDK Setup
```ts
// lib/amplitude-analytics.ts
import { Client } from 'amplitude-analytics'
