---
name: integrate-postmark-email
description: Use when integrating Postmark email into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: transactional, high deliverability.
---

# Integrate Postmark email

Production integration guide for Postmark email.

## What Postmark email Provides
transactional, high deliverability

## Setup
1. Create account / project on Postmark email
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install postmark_email` (or equivalent)

## SDK Setup
```ts
// lib/postmark-email.ts
import { Client } from 'postmark-email'

export const client = new Client({
  apiKey: process.env.POSTMARK_EMAIL_API_KEY!,
})
```
