---
name: integrate-mixpanel-analytics
description: Use when integrating Mixpanel analytics into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: product analytics, funnels, retention.
---

# Integrate Mixpanel analytics

Production integration guide for Mixpanel analytics.

## What Mixpanel analytics Provides
product analytics, funnels, retention

## Setup
1. Create account / project on Mixpanel analytics
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install mixpanel_analytics` (or equivalent)

## SDK Setup
```ts
// lib/mixpanel-analytics.ts
import { Client } from 'mixpanel-analytics'

export const client = new Client({
  apiKey: process.env.MIXPANEL_ANALYTICS_API_KEY!,
})
```
