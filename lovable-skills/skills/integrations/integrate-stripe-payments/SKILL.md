...
description: Use when integrating Stripe payments into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: checkout, subscriptions, webhooks, customer portal, tax.
---

# Integrate Stripe payments

Production integration guide for Stripe payments.

## What Stripe payments Provides
checkout, subscriptions, webhooks, customer portal, tax

## Setup
1. Create account / project on Stripe payments
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install stripe_payments` (or equivalent)

## SDK Setup
```ts
// lib/stripe-payments.ts
import { Client } from 'stripe-payments'

export const client = new Client({
  apiKey: process.env.STRIPE_PAYMENTS_API_KEY!,
})
```
