---
name: integrate-appwrite-backend
description: Use when integrating Appwrite backend into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: auth, database, storage, functions, realtime.
---

# Integrate Appwrite backend

Production integration guide for Appwrite backend.

## What Appwrite backend Provides
auth, database, storage, functions, realtime

## Setup
1. Create account / project on Appwrite backend
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install appwrite_backend` (or equivalent)

## SDK Setup
```ts
// lib/appwrite-backend.ts
import { Client } from 'appwrite-backend'

export const client = new Client({
  apiKey: process.env.APPWRITE_BACKEND_API_KEY!,
})
```
