---
name: integrate-notion-api
description: Use when integrating Notion API into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: pages, databases, blocks.
---

# Integrate Notion API

Production integration guide for Notion API.

## What Notion API Provides
pages, databases, blocks

## Setup
1. Create account / project on Notion API
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install notion_api` (or equivalent)
...
export const client = new Client({
  apiKey: process.env.NOTION_API_API_KEY!,
})
```
