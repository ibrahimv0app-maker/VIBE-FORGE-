---
name: integrate-intercom-api
description: Use when integrating Intercom API into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: customer messaging, tickets.
---

# Integrate Intercom API

Production integration guide for Intercom API.

## What Intercom API Provides
customer messaging, tickets

## Setup
1. Create account / project on Intercom API
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install intercom_api` (or equivalent)

## SDK Setup
```ts
// lib/intercom-api.ts
import { Client } from 'intercom-api'

export const client = new Client({
  apiKey: process.env.INTERCOM_API_API_KEY!,
})
```
