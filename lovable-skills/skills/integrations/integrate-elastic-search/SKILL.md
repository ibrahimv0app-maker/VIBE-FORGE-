---
name: integrate-elastic-search
description: Use when integrating Elasticsearch into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: full-text, analytics, log search.
---

# Integrate Elasticsearch

Production integration guide for Elasticsearch.

## What Elasticsearch Provides
full-text, analytics, log search

## Setup
1. Create account / project on Elasticsearch
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install elastic_search` (or equivalent)

## SDK Setup
```ts
// lib/elastic-search.ts
