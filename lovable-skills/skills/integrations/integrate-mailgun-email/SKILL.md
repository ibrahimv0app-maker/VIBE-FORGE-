---
name: integrate-mailgun-email
description: Use when integrating Mailgun email into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: transactional, marketing, EU region.
---

# Integrate Mailgun email

Production integration guide for Mailgun email.

## What Mailgun email Provides
transactional, marketing, EU region

## Setup
1. Create account / project on Mailgun email
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install mailgun_email` (or equivalent)

## SDK Setup
```ts
// lib/mailgun-email.ts
import { Client } from 'mailgun-email'
