---
name: integrate-i18next-localization
description: Use when integrating i18next localization into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: React / JS i18n library.
---

# Integrate i18next localization

Production integration guide for i18next localization.

## What i18next localization Provides
React / JS i18n library

## Setup
1. Create account / project on i18next localization
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install i18next_localization` (or equivalent)

## SDK Setup
```ts
// lib/i18next-localization.ts
import { Client } from 'i18next-localization'

export const client = new Client({
  apiKey: process.env.I18NEXT_LOCALIZATION_API_KEY!,
})
```
