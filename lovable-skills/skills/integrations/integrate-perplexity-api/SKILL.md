---
name: integrate-perplexity-api
description: Use when integrating Perplexity API into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: LLM with web search.
---

# Integrate Perplexity API

Production integration guide for Perplexity API.

## What Perplexity API Provides
LLM with web search

## Setup
1. Create account / project on Perplexity API
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install perplexity_api` (or equivalent)

## SDK Setup
```ts
// lib/perplexity-api.ts
import { Client } from 'perplexity-api'

export const client = new Client({
  apiKey: process.env.PERPLEXITY_API_API_KEY!,
})
```
