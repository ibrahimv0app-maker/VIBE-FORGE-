---
name: integrate-linear-api
description: Use when integrating Linear API into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: issue tracking, projects, cycles.
---

# Integrate Linear API

Production integration guide for Linear API.

## What Linear API Provides
issue tracking, projects, cycles

## Setup
1. Create account / project on Linear API
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install linear_api` (or equivalent)

## SDK Setup
```ts
// lib/linear-api.ts
import { Client } from 'linear-api'

export const client = new Client({
  apiKey: process.env.LINEAR_API_API_KEY!,
})
```
