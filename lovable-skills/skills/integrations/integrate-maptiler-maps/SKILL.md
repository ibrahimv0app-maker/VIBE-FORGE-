---
name: integrate-maptiler-maps
description: Use when integrating MapTiler maps into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: open-source maps, vector tiles.
---

# Integrate MapTiler maps

Production integration guide for MapTiler maps.

## What MapTiler maps Provides
open-source maps, vector tiles

## Setup
1. Create account / project on MapTiler maps
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install maptiler_maps` (or equivalent)

## SDK Setup
```ts
// lib/maptiler-maps.ts
import { Client } from 'maptiler-maps'

export const client = new Client({
  apiKey: process.env.MAPTILER_MAPS_API_KEY!,
})
```
