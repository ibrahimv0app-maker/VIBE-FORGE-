...
description: Use when integrating Firebase authentication into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: email, phone, OAuth, custom claims.
---

# Integrate Firebase authentication

Production integration guide for Firebase authentication.

## What Firebase authentication Provides
email, phone, OAuth, custom claims

## Setup
1. Create account / project on Firebase authentication
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install firebase_auth` (or equivalent)

## SDK Setup
```ts
// lib/firebase-auth.ts
import { Client } from 'firebase-auth'
