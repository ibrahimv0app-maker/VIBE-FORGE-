---
name: integrate-anthropic-claude-3
description: Use when integrating Anthropic Claude 3 into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: Haiku, Sonnet, Opus, vision.
---

# Integrate Anthropic Claude 3

Production integration guide for Anthropic Claude 3.

## What Anthropic Claude 3 Provides
Haiku, Sonnet, Opus, vision

## Setup
1. Create account / project on Anthropic Claude 3
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install anthropic_claude_3` (or equivalent)

## SDK Setup
```ts
// lib/anthropic-claude-3.ts
import { Client } from 'anthropic-claude-3'

export const client = new Client({
  apiKey: process.env.ANTHROPIC_CLAUDE_3_API_KEY!,
})
```
