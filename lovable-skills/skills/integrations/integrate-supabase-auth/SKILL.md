...
description: Use when integrating Supabase authentication into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: email, magic link, OAuth, RLS.
---

# Integrate Supabase authentication

Production integration guide for Supabase authentication.

## What Supabase authentication Provides
email, magic link, OAuth, RLS

## Setup
1. Create account / project on Supabase authentication
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install supabase_auth` (or equivalent)

## SDK Setup
```ts
// lib/supabase-auth.ts
import { Client } from 'supabase-auth'

export const client = new Client({
  apiKey: process.env.SUPABASE_AUTH_API_KEY!,
})
```
