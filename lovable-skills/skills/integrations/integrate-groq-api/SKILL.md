---
name: integrate-groq-api
description: Use when integrating Groq API into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: ultra-fast LLM inference.
---

# Integrate Groq API

Production integration guide for Groq API.

## What Groq API Provides
ultra-fast LLM inference

## Setup
1. Create account / project on Groq API
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install groq_api` (or equivalent)

## SDK Setup
```ts
// lib/groq-api.ts
import { Client } from 'groq-api'

export const client = new Client({
  apiKey: process.env.GROQ_API_API_KEY!,
})
```
