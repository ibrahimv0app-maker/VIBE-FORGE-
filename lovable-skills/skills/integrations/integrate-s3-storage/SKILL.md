---
name: integrate-s3-storage
description: Use when integrating AWS S3 storage into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: object storage, presigned URLs, lifecycle.
---

# Integrate AWS S3 storage

Production integration guide for AWS S3 storage.

## What AWS S3 storage Provides
object storage, presigned URLs, lifecycle

## Setup
1. Create account / project on AWS S3 storage
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install s3_storage` (or equivalent)

## SDK Setup
```ts
// lib/s3-storage.ts
import { Client } from 's3-storage'

export const client = new Client({
  apiKey: process.env.S3_STORAGE_API_KEY!,
})
```
