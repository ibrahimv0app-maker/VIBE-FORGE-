---
name: integrate-sendgrid-email
description: Use when integrating SendGrid email into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: transactional, marketing, templates.
---

# Integrate SendGrid email

Production integration guide for SendGrid email.

## What SendGrid email Provides
transactional, marketing, templates

## Setup
1. Create account / project on SendGrid email
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install sendgrid_email` (or equivalent)

## SDK Setup
```ts
// lib/sendgrid-email.ts
import { Client } from 'sendgrid-email'
