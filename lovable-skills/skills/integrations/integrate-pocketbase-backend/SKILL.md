---
name: integrate-pocketbase-backend
description: Use when integrating Pocketbase backend into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: self-hosted BaaS, SQLite, auth, realtime.
---

# Integrate Pocketbase backend

Production integration guide for Pocketbase backend.

## What Pocketbase backend Provides
self-hosted BaaS, SQLite, auth, realtime

## Setup
1. Create account / project on Pocketbase backend
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install pocketbase_backend` (or equivalent)

## SDK Setup
```ts
// lib/pocketbase-backend.ts
import { Client } from 'pocketbase-backend'

export const client = new Client({
  apiKey: process.env.POCKETBASE_BACKEND_API_KEY!,
})
```
