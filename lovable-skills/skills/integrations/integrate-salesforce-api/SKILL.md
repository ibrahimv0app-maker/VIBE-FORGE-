---
name: integrate-salesforce-api
description: Use when integrating Salesforce API into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: CRM, SOQL, objects.
---

# Integrate Salesforce API

Production integration guide for Salesforce API.

## What Salesforce API Provides
CRM, SOQL, objects

## Setup
1. Create account / project on Salesforce API
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install salesforce_api` (or equivalent)

## SDK Setup
```ts
// lib/salesforce-api.ts
import { Client } from 'salesforce-api'

export const client = new Client({
  apiKey: process.env.SALESFORCE_API_API_KEY!,
})
```
