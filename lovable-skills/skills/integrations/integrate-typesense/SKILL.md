---
name: integrate-typesense
description: Use when integrating Typesense self-hosted search into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: open-source, typo-tolerance.
---

# Integrate Typesense self-hosted search

Production integration guide for Typesense self-hosted search.

## What Typesense self-hosted search Provides
open-source, typo-tolerance

## Setup
1. Create account / project on Typesense self-hosted search
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install typesense` (or equivalent)
