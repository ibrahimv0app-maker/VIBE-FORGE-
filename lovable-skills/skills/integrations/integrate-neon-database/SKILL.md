---
name: integrate-neon-database
description: Use when integrating Neon database into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: serverless Postgres, branching.
---

# Integrate Neon database

Production integration guide for Neon database.

## What Neon database Provides
serverless Postgres, branching

## Setup
1. Create account / project on Neon database
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install neon_database` (or equivalent)

## SDK Setup
```ts
// lib/neon-database.ts
import { Client } from 'neon-database'

export const client = new Client({
  apiKey: process.env.NEON_DATABASE_API_KEY!,
})
```
