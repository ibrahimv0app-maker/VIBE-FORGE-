---
name: integrate-datadog-monitoring
description: Use when integrating Datadog monitoring into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: metrics, traces, logs, APM.
---

# Integrate Datadog monitoring

Production integration guide for Datadog monitoring.

## What Datadog monitoring Provides
metrics, traces, logs, APM

## Setup
1. Create account / project on Datadog monitoring
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install datadog_monitoring` (or equivalent)

## SDK Setup
```ts
// lib/datadog-monitoring.ts
import { Client } from 'datadog-monitoring'

export const client = new Client({
  apiKey: process.env.DATADOG_MONITORING_API_KEY!,
})
```
