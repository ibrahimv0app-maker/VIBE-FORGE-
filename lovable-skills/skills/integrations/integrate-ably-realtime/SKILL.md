---
name: integrate-ably-realtime
description: Use when integrating Ably realtime into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: managed realtime, edge.
---

# Integrate Ably realtime

Production integration guide for Ably realtime.

## What Ably realtime Provides
managed realtime, edge

## Setup
1. Create account / project on Ably realtime
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install ably_realtime` (or equivalent)

## SDK Setup
```ts
// lib/ably-realtime.ts
import { Client } from 'ably-realtime'

export const client = new Client({
  apiKey: process.env.ABLY_REALTIME_API_KEY!,
})
```
