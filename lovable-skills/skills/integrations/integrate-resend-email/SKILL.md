---
name: integrate-resend-email
description: Use when integrating Resend email into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: React email templates, transactional.
---

# Integrate Resend email

Production integration guide for Resend email.

## What Resend email Provides
React email templates, transactional

## Setup
1. Create account / project on Resend email
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install resend_email` (or equivalent)

## SDK Setup
```ts
// lib/resend-email.ts
import { Client } from 'resend-email'
