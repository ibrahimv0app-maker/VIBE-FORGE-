---
name: integrate-paddle-payments
description: Use when integrating Paddle payments into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: Merchant of Record, subscriptions, tax handling.
---

# Integrate Paddle payments

Production integration guide for Paddle payments.

## What Paddle payments Provides
Merchant of Record, subscriptions, tax handling

## Setup
1. Create account / project on Paddle payments
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install paddle_payments` (or equivalent)

## SDK Setup
```ts
// lib/paddle-payments.ts
import { Client } from 'paddle-payments'

export const client = new Client({
  apiKey: process.env.PADDLE_PAYMENTS_API_KEY!,
})
```
