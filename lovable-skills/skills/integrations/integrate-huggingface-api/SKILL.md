---
name: integrate-huggingface-api
description: Use when integrating Hugging Face Inference API into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: transformers, models, spaces.
---

# Integrate Hugging Face Inference API

Production integration guide for Hugging Face Inference API.

## What Hugging Face Inference API Provides
transformers, models, spaces

## Setup
1. Create account / project on Hugging Face Inference API
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install huggingface_api` (or equivalent)

## SDK Setup
```ts
// lib/huggingface-api.ts
import { Client } from 'huggingface-api'

export const client = new Client({
  apiKey: process.env.HUGGINGFACE_API_API_KEY!,
})
```
