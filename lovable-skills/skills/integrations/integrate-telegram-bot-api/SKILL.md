---
name: integrate-telegram-bot-api
description: Use when integrating Telegram Bot API into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: bots, channels, inline keyboards.
---

# Integrate Telegram Bot API

Production integration guide for Telegram Bot API.

## What Telegram Bot API Provides
bots, channels, inline keyboards

## Setup
1. Create account / project on Telegram Bot API
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install telegram_bot_api` (or equivalent)

## SDK Setup
```ts
// lib/telegram-bot-api.ts
import { Client } from 'telegram-bot-api'

export const client = new Client({
  apiKey: process.env.TELEGRAM_BOT_API_API_KEY!,
})
```
