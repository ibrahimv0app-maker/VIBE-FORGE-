---
name: integrate-cloudflare-stream
description: Use when integrating Cloudflare Stream video into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: video encoding, delivery.
---

# Integrate Cloudflare Stream video

Production integration guide for Cloudflare Stream video.

## What Cloudflare Stream video Provides
video encoding, delivery

## Setup
1. Create account / project on Cloudflare Stream video
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install cloudflare_stream` (or equivalent)

## SDK Setup
```ts
// lib/cloudflare-stream.ts
import { Client } from 'cloudflare-stream'

export const client = new Client({
  apiKey: process.env.CLOUDFLARE_STREAM_API_KEY!,
})
```
