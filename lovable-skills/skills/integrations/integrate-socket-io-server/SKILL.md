---
name: integrate-socket-io-server
description: Use when integrating Socket.IO self-hosted into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: websocket with fallback, rooms.
---

# Integrate Socket.IO self-hosted

Production integration guide for Socket.IO self-hosted.

## What Socket.IO self-hosted Provides
websocket with fallback, rooms

## Setup
1. Create account / project on Socket.IO self-hosted
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install socket_io_server` (or equivalent)

## SDK Setup
```ts
// lib/socket-io-server.ts
import { Client } from 'socket-io-server'

export const client = new Client({
  apiKey: process.env.SOCKET_IO_SERVER_API_KEY!,
})
```
