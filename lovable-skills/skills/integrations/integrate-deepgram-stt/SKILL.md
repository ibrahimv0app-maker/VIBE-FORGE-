---
name: integrate-deepgram-stt
description: Use when integrating Deepgram STT into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: speech-to-text, streaming.
---

# Integrate Deepgram STT

Production integration guide for Deepgram STT.

## What Deepgram STT Provides
speech-to-text, streaming

## Setup
1. Create account / project on Deepgram STT
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install deepgram_stt` (or equivalent)

## SDK Setup
```ts
// lib/deepgram-stt.ts
import { Client } from 'deepgram-stt'

export const client = new Client({
  apiKey: process.env.DEEPGRAM_STT_API_KEY!,
})
```
