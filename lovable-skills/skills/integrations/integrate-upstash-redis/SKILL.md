---
name: integrate-upstash-redis
description: Use when integrating Upstash Redis into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: serverless Redis, REST API.
---

# Integrate Upstash Redis

Production integration guide for Upstash Redis.

## What Upstash Redis Provides
serverless Redis, REST API

## Setup
1. Create account / project on Upstash Redis
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install upstash_redis` (or equivalent)
...
export const client = new Client({
  apiKey: process.env.UPSTASH_REDIS_API_KEY!,
})
```
