---
name: integrate-mongodb-atlas
description: Use when integrating MongoDB Atlas into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: managed MongoDB, vector search.
---

# Integrate MongoDB Atlas

Production integration guide for MongoDB Atlas.

## What MongoDB Atlas Provides
managed MongoDB, vector search

## Setup
1. Create account / project on MongoDB Atlas
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install mongodb_atlas` (or equivalent)

## SDK Setup
```ts
// lib/mongodb-atlas.ts
import { Client } from 'mongodb-atlas'
