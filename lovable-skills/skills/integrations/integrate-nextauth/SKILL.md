...
description: Use when integrating NextAuth.js / Auth.js into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: OAuth providers, JWT, sessions, callbacks.
---

# Integrate NextAuth.js / Auth.js

Production integration guide for NextAuth.js / Auth.js.

## What NextAuth.js / Auth.js Provides
OAuth providers, JWT, sessions, callbacks

## Setup
1. Create account / project on NextAuth.js / Auth.js
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install nextauth` (or equivalent)
...
export const client = new Client({
  apiKey: process.env.NEXTAUTH_API_KEY!,
})
```
