---
name: integrate-grafana-cloud
description: Use when integrating Grafana Cloud monitoring into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: Prometheus, Loki, Tempo, alerts.
---

# Integrate Grafana Cloud monitoring

Production integration guide for Grafana Cloud monitoring.

## What Grafana Cloud monitoring Provides
Prometheus, Loki, Tempo, alerts

## Setup
1. Create account / project on Grafana Cloud monitoring
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install grafana_cloud` (or equivalent)

## SDK Setup
```ts
// lib/grafana-cloud.ts
import { Client } from 'grafana-cloud'
