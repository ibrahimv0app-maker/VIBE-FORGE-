---
name: integrate-meilisearch
description: Use when integrating Meilisearch self-hosted search into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: open-source Algolia alternative.
---

# Integrate Meilisearch self-hosted search

Production integration guide for Meilisearch self-hosted search.

## What Meilisearch self-hosted search Provides
open-source Algolia alternative

## Setup
1. Create account / project on Meilisearch self-hosted search
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install meilisearch` (or equivalent)
