---
name: integrate-turso-database
description: Use when integrating Turso database into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: SQLite at edge, replication.
---

# Integrate Turso database

Production integration guide for Turso database.

## What Turso database Provides
SQLite at edge, replication

## Setup
1. Create account / project on Turso database
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install turso_database` (or equivalent)

## SDK Setup
```ts
// lib/turso-database.ts
import { Client } from 'turso-database'

export const client = new Client({
  apiKey: process.env.TURSO_DATABASE_API_KEY!,
})
```
