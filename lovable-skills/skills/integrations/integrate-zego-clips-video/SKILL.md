---
name: integrate-zego-clips-video
description: Use when integrating Zego video into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: real-time video, live streaming.
---

# Integrate Zego video

Production integration guide for Zego video.

## What Zego video Provides
real-time video, live streaming

## Setup
1. Create account / project on Zego video
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install zego_clips_video` (or equivalent)

## SDK Setup
```ts
// lib/zego-clips-video.ts
import { Client } from 'zego-clips-video'

export const client = new Client({
  apiKey: process.env.ZEGO_CLIPS_VIDEO_API_KEY!,
})
```
