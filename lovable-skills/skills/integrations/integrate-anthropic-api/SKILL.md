...
description: Use when integrating Anthropic Claude API into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: Claude, vision, tools.
---

# Integrate Anthropic Claude API

Production integration guide for Anthropic Claude API.

## What Anthropic Claude API Provides
Claude, vision, tools

## Setup
1. Create account / project on Anthropic Claude API
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install anthropic_api` (or equivalent)

## SDK Setup
```ts
// lib/anthropic-api.ts
import { Client } from 'anthropic-api'

export const client = new Client({
  apiKey: process.env.ANTHROPIC_API_API_KEY!,
})
```
