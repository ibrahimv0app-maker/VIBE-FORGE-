---
name: integrate-algolia-search
description: Use when integrating Algolia search into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: indexing, instant search, faceting, A/B testing.
---

# Integrate Algolia search

Production integration guide for Algolia search.

## What Algolia search Provides
indexing, instant search, faceting, A/B testing

## Setup
1. Create account / project on Algolia search
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install algolia_search` (or equivalent)

## SDK Setup
```ts
// lib/algolia-search.ts
import { Client } from 'algolia-search'
