---
name: integrate-restream-live
description: Use when integrating Restream live video into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: multistream, recording, studio.
---

# Integrate Restream live video

Production integration guide for Restream live video.

## What Restream live video Provides
multistream, recording, studio

## Setup
1. Create account / project on Restream live video
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install restream_live` (or equivalent)

## SDK Setup
```ts
// lib/restream-live.ts
import { Client } from 'restream-live'

export const client = new Client({
  apiKey: process.env.RESTREAM_LIVE_API_KEY!,
})
```
