---
name: integrate-firebase-backend
description: Use when integrating Firebase backend into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: Firestore, auth, storage, functions, messaging.
---

# Integrate Firebase backend

Production integration guide for Firebase backend.

## What Firebase backend Provides
Firestore, auth, storage, functions, messaging

## Setup
1. Create account / project on Firebase backend
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install firebase_backend` (or equivalent)

## SDK Setup
```ts
// lib/firebase-backend.ts
import { Client } from 'firebase-backend'

export const client = new Client({
  apiKey: process.env.FIREBASE_BACKEND_API_KEY!,
})
```
