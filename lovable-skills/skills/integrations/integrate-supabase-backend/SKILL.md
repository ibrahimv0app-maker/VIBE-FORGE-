---
name: integrate-supabase-backend
description: Use when integrating Supabase backend into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: Postgres, auth, storage, realtime, edge.
---

# Integrate Supabase backend

Production integration guide for Supabase backend.

## What Supabase backend Provides
Postgres, auth, storage, realtime, edge

## Setup
1. Create account / project on Supabase backend
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install supabase_backend` (or equivalent)

## SDK Setup
```ts
// lib/supabase-backend.ts
import { Client } from 'supabase-backend'

export const client = new Client({
  apiKey: process.env.SUPABASE_BACKEND_API_KEY!,
})
```
