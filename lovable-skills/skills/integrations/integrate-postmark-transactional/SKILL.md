---
name: integrate-postmark-transactional
description: Use when integrating Postmark transactional email into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: transactional templates, inbound.
---

# Integrate Postmark transactional email

Production integration guide for Postmark transactional email.

## What Postmark transactional email Provides
transactional templates, inbound

## Setup
1. Create account / project on Postmark transactional email
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install postmark_transactional` (or equivalent)

## SDK Setup
```ts
// lib/postmark-transactional.ts
import { Client } from 'postmark-transactional'

export const client = new Client({
  apiKey: process.env.POSTMARK_TRANSACTIONAL_API_KEY!,
})
```
