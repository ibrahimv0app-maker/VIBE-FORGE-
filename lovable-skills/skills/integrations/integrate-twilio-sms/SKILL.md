---
name: integrate-twilio-sms
description: Use when integrating Twilio SMS / voice into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: SMS, voice, OTP, verify.
---

# Integrate Twilio SMS / voice

Production integration guide for Twilio SMS / voice.

## What Twilio SMS / voice Provides
SMS, voice, OTP, verify

## Setup
1. Create account / project on Twilio SMS / voice
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install twilio_sms` (or equivalent)

## SDK Setup
```ts
// lib/twilio-sms.ts
import { Client } from 'twilio-sms'

export const client = new Client({
  apiKey: process.env.TWILIO_SMS_API_KEY!,
})
```
