---
name: integrate-aws-ses-email
description: Use when integrating AWS SES email into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: low-cost, high-volume.
---

# Integrate AWS SES email

Production integration guide for AWS SES email.

## What AWS SES email Provides
low-cost, high-volume

## Setup
1. Create account / project on AWS SES email
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install aws_ses_email` (or equivalent)

## SDK Setup
```ts
// lib/aws-ses-email.ts
import { Client } from 'aws-ses-email'

export const client = new Client({
  apiKey: process.env.AWS_SES_EMAIL_API_KEY!,
})
```
