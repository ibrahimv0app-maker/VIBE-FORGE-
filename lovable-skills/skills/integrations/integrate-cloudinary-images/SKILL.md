---
name: integrate-cloudinary-images
description: Use when integrating Cloudinary image CDN into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: transformations, optimization, delivery.
---

# Integrate Cloudinary image CDN

Production integration guide for Cloudinary image CDN.

## What Cloudinary image CDN Provides
transformations, optimization, delivery

## Setup
1. Create account / project on Cloudinary image CDN
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install cloudinary_images` (or equivalent)

## SDK Setup
```ts
// lib/cloudinary-images.ts
import { Client } from 'cloudinary-images'

export const client = new Client({
  apiKey: process.env.CLOUDINARY_IMAGES_API_KEY!,
})
```
