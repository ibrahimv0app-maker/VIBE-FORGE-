---
name: integrate-mapbox-maps
description: Use when integrating Mapbox maps into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: interactive maps, geocoding, directions.
---

# Integrate Mapbox maps

Production integration guide for Mapbox maps.

## What Mapbox maps Provides
interactive maps, geocoding, directions

## Setup
1. Create account / project on Mapbox maps
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install mapbox_maps` (or equivalent)

## SDK Setup
```ts
// lib/mapbox-maps.ts
import { Client } from 'mapbox-maps'

export const client = new Client({
  apiKey: process.env.MAPBOX_MAPS_API_KEY!,
})
```
