---
name: integrate-posthog-analytics
description: Use when integrating PostHog analytics into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: open-source product analytics, feature flags.
---

# Integrate PostHog analytics

Production integration guide for PostHog analytics.

## What PostHog analytics Provides
open-source product analytics, feature flags

## Setup
1. Create account / project on PostHog analytics
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install posthog_analytics` (or equivalent)

## SDK Setup
```ts
// lib/posthog-analytics.ts
import { Client } from 'posthog-analytics'

export const client = new Client({
  apiKey: process.env.POSTHOG_ANALYTICS_API_KEY!,
})
```
