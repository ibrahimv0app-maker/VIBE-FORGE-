...
description: Use when integrating OpenAI API into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: GPT, DALL-E, Whisper, embeddings.
---

# Integrate OpenAI API

Production integration guide for OpenAI API.

## What OpenAI API Provides
GPT, DALL-E, Whisper, embeddings

## Setup
1. Create account / project on OpenAI API
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install openai_api` (or equivalent)

## SDK Setup
```ts
// lib/openai-api.ts
import { Client } from 'openai-api'
