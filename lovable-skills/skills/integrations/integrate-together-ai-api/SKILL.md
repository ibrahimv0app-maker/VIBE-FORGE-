---
name: integrate-together-ai-api
description: Use when integrating Together AI API into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: open-source LLMs, fine-tuning.
---

# Integrate Together AI API

Production integration guide for Together AI API.

## What Together AI API Provides
open-source LLMs, fine-tuning

## Setup
1. Create account / project on Together AI API
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install together_ai_api` (or equivalent)

## SDK Setup
```ts
// lib/together-ai-api.ts
import { Client } from 'together-ai-api'

export const client = new Client({
  apiKey: process.env.TOGETHER_AI_API_API_KEY!,
})
```
