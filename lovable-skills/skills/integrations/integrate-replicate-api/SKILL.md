---
name: integrate-replicate-api
description: Use when integrating Replicate API into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: open-source ML models, predictions.
---

# Integrate Replicate API

Production integration guide for Replicate API.

## What Replicate API Provides
open-source ML models, predictions

## Setup
1. Create account / project on Replicate API
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install replicate_api` (or equivalent)

## SDK Setup
```ts
// lib/replicate-api.ts
import { Client } from 'replicate-api'

export const client = new Client({
  apiKey: process.env.REPLICATE_API_API_KEY!,
})
```
