---
name: integrate-clerk-auth
description: Use when integrating Clerk authentication into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: drop-in components, organizations, MFA.
---

# Integrate Clerk authentication

Production integration guide for Clerk authentication.

## What Clerk authentication Provides
drop-in components, organizations, MFA

## Setup
1. Create account / project on Clerk authentication
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install clerk_auth` (or equivalent)

## SDK Setup
```ts
// lib/clerk-auth.ts
import { Client } from 'clerk-auth'

export const client = new Client({
  apiKey: process.env.CLERK_AUTH_API_KEY!,
})
```
