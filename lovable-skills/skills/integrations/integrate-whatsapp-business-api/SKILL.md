---
name: integrate-whatsapp-business-api
description: Use when integrating WhatsApp Business API into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: messaging, templates.
---

# Integrate WhatsApp Business API

Production integration guide for WhatsApp Business API.

## What WhatsApp Business API Provides
messaging, templates

## Setup
1. Create account / project on WhatsApp Business API
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install whatsapp_business_api` (or equivalent)

## SDK Setup
```ts
// lib/whatsapp-business-api.ts
import { Client } from 'whatsapp-business-api'

export const client = new Client({
  apiKey: process.env.WHATSAPP_BUSINESS_API_API_KEY!,
})
```
