---
name: integrate-hubspot-api
description: Use when integrating HubSpot API into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: CRM, marketing, sales.
---

# Integrate HubSpot API

Production integration guide for HubSpot API.

## What HubSpot API Provides
CRM, marketing, sales

## Setup
1. Create account / project on HubSpot API
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install hubspot_api` (or equivalent)
...
export const client = new Client({
  apiKey: process.env.HUBSPOT_API_API_KEY!,
})
```
