---
name: integrate-partykit-realtime
description: Use when integrating PartyKit realtime into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: edge websockets, multi-agent.
---

# Integrate PartyKit realtime

Production integration guide for PartyKit realtime.

## What PartyKit realtime Provides
edge websockets, multi-agent

## Setup
1. Create account / project on PartyKit realtime
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install partykit_realtime` (or equivalent)

## SDK Setup
```ts
// lib/partykit-realtime.ts
import { Client } from 'partykit-realtime'

export const client = new Client({
  apiKey: process.env.PARTYKIT_REALTIME_API_KEY!,
})
```
