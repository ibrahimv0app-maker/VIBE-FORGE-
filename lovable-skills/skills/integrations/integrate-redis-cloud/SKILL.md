---
name: integrate-redis-cloud
description: Use when integrating Redis Cloud into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: managed Redis, vector search, streams.
---

# Integrate Redis Cloud

Production integration guide for Redis Cloud.

## What Redis Cloud Provides
managed Redis, vector search, streams

## Setup
1. Create account / project on Redis Cloud
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install redis_cloud` (or equivalent)

## SDK Setup
```ts
// lib/redis-cloud.ts
import { Client } from 'redis-cloud'
