---
name: integrate-slack-api
description: Use when integrating Slack API into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: channels, messages, slash commands.
---

# Integrate Slack API

Production integration guide for Slack API.

## What Slack API Provides
channels, messages, slash commands

## Setup
1. Create account / project on Slack API
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install slack_api` (or equivalent)

## SDK Setup
```ts
// lib/slack-api.ts
import { Client } from 'slack-api'

export const client = new Client({
  apiKey: process.env.SLACK_API_API_KEY!,
})
```
