---
name: integrate-segment-cdp
description: Use when integrating Segment CDP into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: customer data pipeline, destinations.
---

# Integrate Segment CDP

Production integration guide for Segment CDP.

## What Segment CDP Provides
customer data pipeline, destinations

## Setup
1. Create account / project on Segment CDP
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install segment_cdp` (or equivalent)

## SDK Setup
```ts
// lib/segment-cdp.ts
import { Client } from 'segment-cdp'

export const client = new Client({
  apiKey: process.env.SEGMENT_CDP_API_KEY!,
})
```
