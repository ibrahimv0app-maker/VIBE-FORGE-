---
name: integrate-lemonsqueezy-payments
description: Use when integrating Lemon Squeezy payments into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: Merchant of Record, subscriptions, webhooks.
---

# Integrate Lemon Squeezy payments

Production integration guide for Lemon Squeezy payments.

## What Lemon Squeezy payments Provides
Merchant of Record, subscriptions, webhooks

## Setup
1. Create account / project on Lemon Squeezy payments
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install lemonsqueezy_payments` (or equivalent)

## SDK Setup
```ts
// lib/lemonsqueezy-payments.ts
import { Client } from 'lemonsqueezy-payments'

export const client = new Client({
  apiKey: process.env.LEMONSQUEEZY_PAYMENTS_API_KEY!,
})
```
