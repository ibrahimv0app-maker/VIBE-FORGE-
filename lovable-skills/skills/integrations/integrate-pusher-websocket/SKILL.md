---
name: integrate-pusher-websocket
description: Use when integrating Pusher websockets into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: managed realtime, channels.
---

# Integrate Pusher websockets

Production integration guide for Pusher websockets.

## What Pusher websockets Provides
managed realtime, channels

## Setup
1. Create account / project on Pusher websockets
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install pusher_websocket` (or equivalent)

## SDK Setup
```ts
// lib/pusher-websocket.ts
import { Client } from 'pusher-websocket'

export const client = new Client({
  apiKey: process.env.PUSHER_WEBSOCKET_API_KEY!,
})
```
