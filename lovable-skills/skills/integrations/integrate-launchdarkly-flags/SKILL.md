---
name: integrate-launchdarkly-flags
description: Use when integrating LaunchDarkly feature flags into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: flags, experiments, targeting.
---

# Integrate LaunchDarkly feature flags

Production integration guide for LaunchDarkly feature flags.

## What LaunchDarkly feature flags Provides
flags, experiments, targeting

## Setup
1. Create account / project on LaunchDarkly feature flags
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install launchdarkly_flags` (or equivalent)

## SDK Setup
```ts
// lib/launchdarkly-flags.ts
import { Client } from 'launchdarkly-flags'

export const client = new Client({
  apiKey: process.env.LAUNCHDARKLY_FLAGS_API_KEY!,
})
```
