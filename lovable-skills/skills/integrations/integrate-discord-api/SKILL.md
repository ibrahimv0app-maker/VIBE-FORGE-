---
name: integrate-discord-api
description: Use when integrating Discord API into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: bots, webhooks, slash commands.
---

# Integrate Discord API

Production integration guide for Discord API.

## What Discord API Provides
bots, webhooks, slash commands

## Setup
1. Create account / project on Discord API
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install discord_api` (or equivalent)
...
export const client = new Client({
  apiKey: process.env.DISCORD_API_API_KEY!,
})
```
