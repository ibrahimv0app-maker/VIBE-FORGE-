...
description: Use when integrating GitHub API into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: repos, issues, PRs, actions.
---

# Integrate GitHub API

Production integration guide for GitHub API.

## What GitHub API Provides
repos, issues, PRs, actions

## Setup
1. Create account / project on GitHub API
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install github_api` (or equivalent)

## SDK Setup
```ts
// lib/github-api.ts
import { Client } from 'github-api'

export const client = new Client({
  apiKey: process.env.GITHUB_API_API_KEY!,
})
```
