---
name: integrate-vonage-sms
description: Use when integrating Vonage SMS / voice into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: SMS, voice, video.
---

# Integrate Vonage SMS / voice

Production integration guide for Vonage SMS / voice.

## What Vonage SMS / voice Provides
SMS, voice, video

## Setup
1. Create account / project on Vonage SMS / voice
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install vonage_sms` (or equivalent)

## SDK Setup
```ts
// lib/vonage-sms.ts
import { Client } from 'vonage-sms'
