---
name: integrate-cockroachdb
description: Use when integrating CockroachDB into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: distributed SQL, multi-region.
---

# Integrate CockroachDB

Production integration guide for CockroachDB.

## What CockroachDB Provides
distributed SQL, multi-region

## Setup
1. Create account / project on CockroachDB
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install cockroachdb` (or equivalent)

## SDK Setup
```ts
// lib/cockroachdb.ts
import { Client } from 'cockroachdb'

export const client = new Client({
  apiKey: process.env.COCKROACHDB_API_KEY!,
})
```
