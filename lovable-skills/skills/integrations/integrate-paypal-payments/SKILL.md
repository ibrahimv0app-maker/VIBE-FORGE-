---
name: integrate-paypal-payments
description: Use when integrating PayPal payments into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: checkout, subscriptions, webhooks.
---

# Integrate PayPal payments

Production integration guide for PayPal payments.

## What PayPal payments Provides
checkout, subscriptions, webhooks

## Setup
1. Create account / project on PayPal payments
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install paypal_payments` (or equivalent)

## SDK Setup
```ts
// lib/paypal-payments.ts
import { Client } from 'paypal-payments'

export const client = new Client({
  apiKey: process.env.PAYPAL_PAYMENTS_API_KEY!,
})
```
