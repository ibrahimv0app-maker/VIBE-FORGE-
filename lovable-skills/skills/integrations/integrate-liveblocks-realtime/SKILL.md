---
name: integrate-liveblocks-realtime
description: Use when integrating Liveblocks realtime into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: collaborative editing, presence.
---

# Integrate Liveblocks realtime

Production integration guide for Liveblocks realtime.

## What Liveblocks realtime Provides
collaborative editing, presence

## Setup
1. Create account / project on Liveblocks realtime
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install liveblocks_realtime` (or equivalent)

## SDK Setup
```ts
// lib/liveblocks-realtime.ts
import { Client } from 'liveblocks-realtime'

export const client = new Client({
  apiKey: process.env.LIVEBLOCKS_REALTIME_API_KEY!,
})
```
