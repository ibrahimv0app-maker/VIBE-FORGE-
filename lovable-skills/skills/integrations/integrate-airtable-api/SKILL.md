---
name: integrate-airtable-api
description: Use when integrating Airtable API into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: databases, attachments, automations.
---

# Integrate Airtable API

Production integration guide for Airtable API.

## What Airtable API Provides
databases, attachments, automations

## Setup
1. Create account / project on Airtable API
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install airtable_api` (or equivalent)

## SDK Setup
```ts
// lib/airtable-api.ts
import { Client } from 'airtable-api'

export const client = new Client({
  apiKey: process.env.AIRTABLE_API_API_KEY!,
})
```
