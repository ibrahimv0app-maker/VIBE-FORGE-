---
name: integrate-whisper-stt
description: Use when integrating OpenAI Whisper STT into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: open-source speech-to-text.
---

# Integrate OpenAI Whisper STT

Production integration guide for OpenAI Whisper STT.

## What OpenAI Whisper STT Provides
open-source speech-to-text

## Setup
1. Create account / project on OpenAI Whisper STT
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install whisper_stt` (or equivalent)

## SDK Setup
```ts
// lib/whisper-stt.ts
import { Client } from 'whisper-stt'

export const client = new Client({
  apiKey: process.env.WHISPER_STT_API_KEY!,
})
```
