---
name: integrate-assemblyai-stt
description: Use when integrating AssemblyAI STT into an app. Always uses the official SDK, handles webhooks, errors, retries, and follows the provider's best practices. Features: speech-to-text, summarization.
---

# Integrate AssemblyAI STT

Production integration guide for AssemblyAI STT.

## What AssemblyAI STT Provides
speech-to-text, summarization

## Setup
1. Create account / project on AssemblyAI STT
2. Get API credentials (keys, tokens, OAuth client)
3. Add credentials to environment variables (never commit)
4. Install official SDK: `npm install assemblyai_stt` (or equivalent)

## SDK Setup
```ts
// lib/assemblyai-stt.ts
import { Client } from 'assemblyai-stt'
